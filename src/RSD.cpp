...
RSD::RSD()
{
	...
	setLogMethod(LogUnit::getGlobalLogMethod());
...
int RSD::start(int argc, char** argv)
{
	...
	try
	{
		while(( c = getopt(argc, argv, "dhf:p:l:w:")) != -1)
		{
			switch(c)
			{
				case 'd':
				...
				case 'h':
					...
					cout << "-d \t\t Server als daemon starten.\n";
					...
					cout << "-w [OPTION] \t Konfiguriert die Logausgabe.\n";
					cout << "\t\t Option s : Logausgabe via syslog.\n";
					cout << "\t\t Option p : Logausgabe auf Standardausgabe.\n\n";
				...
				case 'w':
					wvalue = optarg;
					if(wvalue[0] == 's')
						LogUnit::setGlobalLogMethod(SYSLOG_LOG);
					else if(wvalue[0] == 'p')
						LogUnit::setGlobalLogMethod(PRINTF_LOG);
